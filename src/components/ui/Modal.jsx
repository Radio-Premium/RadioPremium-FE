...
const Modal = ({ children, title, subTitle }) => {
  return (
    <div className="fixed inset-0 z-500 bg-white/30 backdrop-blur-sm">
      <div className="absolute top-[50%] left-[50%] z-600 translate-x-[-50%] translate-y-[-50%] rounded-xl bg-white p-4 px-[25px] py-[20px] shadow-md">
        <div className="pb-[10px]">
          <WarningIcon className="mx-auto h-[45px] w-[45px]" />
        </div>
        <>
          <p className="text-center text-[20px] font-bold text-black">
            {title}
          </p>
